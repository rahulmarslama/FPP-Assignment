...
public class MyBST
{
    ...
    //1. Preorder is traversal with VLR (Visit Left Right)
    ...
    private static Node lastNodeInPreOrder(Node node)
    {
        ...
        {
            return lastNodeInPreOrder(node.right);
        }
        else
        {
            return lastNodeInPreOrder(node.left);
        }

    }
    private void preOrder(Node node,StringBuilder str)
    {

        Node lastNode = lastNodeInPreOrder(root);
        if(node == lastNode)
        {
    ...
    //2. PostOrder is traversal with LRV (Left Right Visit)
    ...
    private void postOrder(Node node,StringBuilder str)
    {
        if((node.left == null && node.right == null)
                || (node.left==null) && (node.right!=null && node.right.NodeTraversed)
                || (node.right==null) && (node.left!=null && node.left.NodeTraversed)
                || ((node.left!=null && node.left.NodeTraversed) && (node.right!=null && node.right.NodeTraversed)))
        {
            ...
            {
                parentNode.left.NodeTraversed = true;
            }
            else
            {
                parentNode.right.NodeTraversed = true;
            }
            postOrder(parentNode,str);
        }
        else if(node.left!=null && !node.left.NodeTraversed)
        {
            postOrder(node.left,str);
        }
        else if(node.right!=null && !node.right.NodeTraversed)
        {
    ...
    //5. Returns the count of leaf nodes;
    ...
    private int leafNodes(Node node)
    {
        ...
        count = count + leafNodes(current.left);
        count = count + leafNodes(current.right);
        return count;
    }
    ...
    public int size()
    {
        List<Integer> traversalValue= asList();
        return traversalValue.size();
    }
    ...
    private Integer findMax(Node t) {
        Node s = findMaxNode(t);
        return s.data;
    }

    private Node findMaxNode(Node t) {
        if(t==null) return null;
        while(t.right != null) {
            t = t.right;
        }
    ...
    private Node parent(Node t, Node n, Node parent) {
        ...
        Integer val = t.data;
        if(n.data.equals(val)) return parent; // Root has no parent
    ...
    static void main(String[] args){
        ...
        bst.postOrder();
        System.out.println(bst.contains(85));
        System.out.println("Leaf Nodes count: "+bst.leafNodes());
        System.out.println("Nodes count: "+bst.size());
